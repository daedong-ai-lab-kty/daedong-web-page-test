...
from graphrag_lib.graphrag.query.structured_search.local_search.na_system_prompt import (
    NA_LOCAL_SEARCH_SYSTEM_PROMPT,
)
...
async def run_global_search(
        ...
        async def generate_streaming_response():
            ...
            async for stream_chunk in api.global_search_streaming(
                ...
                if get_context_data:
                    ...
                    # print('- Stream chunk:', stream_chunk)
                    yield f"{stream_chunk}"

        # return asyncio.run(generate_streaming_response())
    ...
    #     )
    # )
    response, context_data = api.global_search(
            config=config,
            nodes=final_nodes,
            entities=final_entities,
            community_reports=final_community_reports,
...
async def run_local_search(
    ...
    if streaming:
        ...
        # return asyncio.run(run_streaming_search())
        # # return asyncio.wait([run_streaming_search()])
...
async def _resolve_parquet_files(
    ...
    for parquet_file in parquet_list:
        ...
        # df_value = _load_table_from_storage(name=parquet_file, storage=storage_obj)
        df_value = await _load_table_from_storage(name=parquet_file, storage=storage_obj)
    ...
    for optional_file in optional_list:
        ...
        # file_exists = storage_obj.has(optional_file)
        ...
        if file_exists:
            ...
            # )
            # df_value = _load_table_from_storage(name=optional_file, storage=storage_obj)
            df_value = await _load_table_from_storage(name=parquet_file, storage=storage_obj)
            dataframe_dict[df_key] = df_value
        else:
