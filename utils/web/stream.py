...
async def async_stream_generator(domain: str, async_data_source):
    """
...
#     global g_stream_gen
#     async for data in g_stream_gen(): 
#         # print(f"Consumed: {data}")
#         yield f"{data}"

# def sync_stream_gen(domain):
#     loop = asyncio.new_event_loop()
#     asyncio.set_event_loop(loop)
#     async_gen = consume_async_data()

#     try:
#         # 첫 번째 청크를 먼저 보냄 (빈 콘텐츠로 초기 설정)
...
#             ],
#         }
#         yield ("data: " + json.dumps(initial_chunk, ensure_ascii=False) + "\n\n").encode('utf-8')
...
#                 # 마지막 데이터 전송 전에 finish_reason을 "stop"으로 설정
#                 current_time = datetime.datetime.now()
#                 timestamp = int(current_time.timestamp())
#                 chunk = {
#                     "id": 'chatcmpl-RAG',
...
#                 # 버퍼가 일정 크기 이상이면 전송
#                 if len(buffer) >= 4:  # 데이터가 일정 크기 이상 누적되면 보냄
#                     current_time = datetime.datetime.now()
#                     timestamp = int(current_time.timestamp())
#                     chunk = {
#                         "id": 'chatcmpl-RAG',
